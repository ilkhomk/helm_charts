...
# Upgrade nginx ingress helm charts to 1.25.1

Current nginx-ingress helm chart is v1.24.0 and we have upgraded to v1.25.1. We upgraded to this version because it is compatible with our current Kubernetes version v1.15.0. To upgrade to nginx-ingress v1.41.2, Kubernetes will need to be v1.16.0. [Ticket to Kubernetes v1.16.0 upgrade request.](https://github.com/fuchicorp/common_tools/issues/321)

## Steps taken to upgrade to nginx-ingress v1.25.1
1. Removed the ingress-nginx chart from /common_tools/charts/ directory <br>
`rm -rf ingress-controller`<br>
2. Add the stable/nginx-ingress<br>
`helm repo add stable https://kubernetes-charts.storage.googleapis.com/`<br>
3. To update helm repos<br>
`helm repo update`<br>
4. Listed all helm chart versions available for stable/nginx-ingress<br>
`helm search -l stable/nginx-ingress`<br>
5. Fetched the chart version 1.25.1<br>
`helm fetch --untar stable/nginx-ingress --version 1.25.1`<br>
6. We move the folder to rename it to original folder name<br>
`mv nginx-ingress/ ingress-controller`<br>
7. The following updates need to be added the value.yaml file below: <br>
*  Under config update to the following:
```
 config: 
...
```
*  Under publishService update to the following:
 ```
   publishService:
    enabled: true
    ...
    pathOverride: ""
```
*  Under externalTrafficPolicy update to the following:
```
...
Error received when we tried to upgrade to v1.25.2 and higher.<br><br>

Note when upgrading to nginx-ingress v1.41.2, you will need to use the repo below because stable/ingress-nginx chart is DEPRECATED<br>
